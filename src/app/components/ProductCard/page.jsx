...
const ProductCard = ({ products }) => {
  return (
    <div className="w-full flex gap-6 justify-center items-center flex-wrap">
      {products.map((item) => (
        <div className="w-[320px] border overflow-hidden rounded-xl" key={item.id}>
          ...
          <div className="btn w-full text-center ">
            <button className="w-full p-4 bg-indigo-500 text-white hover:bg-indigo-400">Add to Cart</button>
          </div>
        </div>
      ))}
    </div>
